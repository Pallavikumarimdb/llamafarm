...
#   timestamp: 2025-10-24T17:15:21+00:00
...
class Parser(BaseModel):
    ...
    type: Optional[
        constr(pattern=r"^[A-Za-z][A-Za-z0-9_]*Parser[A-Za-z0-9_]*$|^DirectoryParser$")
    ] = Field(
        None, description="Parser type identifier (e.g., PDFParser_PyPDF2, TextParser_LlamaIndex)"
    )
...
class Server(BaseModel):
    ...
    command: Optional[str] = Field(
        None, description="Command/binary to launch the MCP server (stdio)"
    )
    args: Optional[list[str]] = Field(None, description="Optional args for the stdio command")
    env: Optional[dict[str, str]] = Field(
        None, description="Environment variables for the stdio command"
    )
    base_url: Optional[str] = Field(None, description="Base URL of the MCP server (http)")
    headers: Optional[dict[str, str]] = Field(
        None,
        description="HTTP headers for the MCP server",
        examples=[{"Authorization": "Bearer ${env:MCP_SERVER_TOKEN}"}],
    )


class Mcp(BaseModel):
    servers: Optional[list[Server]] = Field(
        None, description="List of MCP Servers available to the project during inference"
    )
...
class LlamaFarmConfig(BaseModel):
    ...
    prompts: Optional[list[Prompt]] = Field([], description="List of named prompt sets")
